...
opdGame.init=function(){
	if(!opdGame.active){
		...
		if(opdGame.model.touchMode)opdGame.stage.enableMouseOver(0);
...
oG.model={
	...
	/*resFolder:'http://localhost:8080/main/games/span/res/',
	preLoaderImageSrc:'http://localhost:8080/main/games/span/res/siteLogo.png',
	contentFolder:'http://localhost:8080/main/content_2/',
	scoresUrl:'../scores/',*/

	/*resFolder:'http://localhost:8080/sites/spanishinflow/src/dist/res/',
	preLoaderImageSrc:'http://localhost:8080/sites/spanishinflow/src/dist/res/siteLogo.png',
	contentFolder:'http://localhost:8080/sites/spanishinflow/src/dist/content_2/',
	scoresUrl:'http://localhost:8080/sites/spanishinflow/src/dist/scores/',*/

	resFolder:'http://188.166.45.159/res/',
	preLoaderImageSrc:'http://188.166.45.159/res/siteLogo.png',
	contentFolder:'http://188.166.45.159/content_2/',
	scoresUrl:'http://188.166.45.159/scores/',

	//scoresUrl:'https://www.spanishinflow.com/scores/',
	...
	//scoresUrl:'https://www.gamestolearnenglish.com/HS/scores_all.php',
	scoresTable:'spanish_scores',

	gameType:0,
/*
	loadScoresMethod:'nodejs',// nodejs or php
	//'movesTime','scoreMoves','scoreTime','timeRound','score','time'
	highScoreTableType:'score',
	highScoreType:0,//needed for php
	scoresUrl:'https://www.gamestolearnenglish.com/HS/scores_all.php',//php
	getScoresUrl:'http://167.99.176.233',//nodejs
...
(function(oG){
	...
	function getScoresLoc(getType,playerName,playerLocation){
		var scrsSrc=oG.model.scoresUrl+'scores_'+oG.model.contentCode+'.json';
		myReq=new XMLHttpRequest();
		myReq.addEventListener('readystatechange',gotScores);
		myReq.open('GET',scrsSrc,true);
...
(function(oG){//checked
	...
	p.setup=function(){
		...
		this.routineDelay=new OpdTimer(this.routineFun);
		this.endDelay=new OpdTimer(this.endScreenFun);
...
/////////////////////////////
//put soap on shelf next to shower head needs bigger target area
...
(function(oG){//checked
	...
	p.initialSetup=function(){
		...
		this.entryPaneContainer.addChild(this.scoreLabel1,this.scoreDisp1);

		if(this.paneLength=='long'){
			this.entryPaneContainer.addChild(this.scoreLabel2,this.scoreDisp2);
		}
	...
	p.setupDisplay=function(){
		...
		if(oG.model.orientation===0){
			...
			if(this.paneLength=='long'){
				...
				opdLib.posItem(this.submitBut,0,164);
				opdLib.posItem(this.nameLabel,0,-90);
				opdLib.posItem(this.localLabel,0,-30);
				opdLib.posItem(this.scoreLabel1,0,30);
				opdLib.posItem(this.scoreDisp1,0,55);
				opdLib.posItem(this.scoreLabel2,0,85);
				opdLib.posItem(this.scoreDisp2,0,110);
				...
				opdLib.posItem(this.scoreLabel1,0,55);
				opdLib.posItem(this.scoreDisp1,0,80);
			...
			if(this.paneLength=='long'){
				...
				opdLib.posItem(this.nameLabel,-160,-8);
				opdLib.posItem(this.localLabel,-50,-8);
				opdLib.posItem(this.scoreLabel1,60,-8);
				opdLib.posItem(this.scoreDisp1,60,18);
				opdLib.posItem(this.scoreLabel2,150,-8);
				opdLib.posItem(this.scoreDisp2,150,18);
				...
				opdLib.posItem(this.scoreLabel1,110,-8);
				opdLib.posItem(this.scoreDisp1,110,18);
		...
		this.entryPaneBack.graphics.beginFill(this.paneBorderColor).drawRoundRect(-paneWidth/2,-paneHeight/2,paneWidth,paneHeight,16);
...
var opdLib={
	...
	fadeOutClear:function($item,$time,$delay){
		createjs.Tween.get($item,{override:true}).wait($delay).to({alpha:0},$time).call(function(){$item.visible=false;});
	},
	...
	scaleImageRect:function(im,maxWid,maxHei){
		var myRatio=maxHei/im.image.height;
		var wRat=maxWid/im.image.width;
		if(wRat<myRatio)myRatio=wRat;
		im.scaleX=im.scaleY=myRatio;
		im.x=(maxWid-(im.image.width*myRatio))/2;
		im.y=(maxHei-(im.image.height*myRatio))/2;
	},
	...
	getPositionInArray:function(gPos,gArr){
		var pos=-1;
	...
	getFileTypeFromEnding:function(file){
...
var opdShapes={
	...
	arrowBord:function(size,color,stroke,strokeWid){
		var outShape=new createjs.Shape();
		outShape.graphics.beginFill(color).beginStroke(stroke).setStrokeStyle(strokeWid);
		outShape.graphics.arc(0,0,size*0.56,-1,1);
		outShape.graphics.arc(-size,size,size*0.40,1,Math.PI);
		outShape.graphics.arc(-size,-size,size*0.40,Math.PI,-1);
		outShape.graphics.closePath();
	...
	tick:function(len,sCol,fCol){
		var hei=len;
		var wid=len/3;
		var rad=len/10;
		var shape=new createjs.Shape();
		shape.graphics.beginFill(fCol).beginStroke(sCol);
		shape.graphics.arc(wid/2,-hei/2,rad,-Math.PI/2,0);
		shape.graphics.arc(wid/2,hei/2,rad,0,Math.PI/2);
		shape.graphics.arc(-wid/2-rad,hei/2,rad,Math.PI/2,Math.PI);
		shape.graphics.arc(-wid/2-rad,hei/2-wid/2+rad/2,rad,Math.PI,-Math.PI/2);
		shape.graphics.arc(-rad/2,hei/2-wid/2-rad/2,0,0,Math.PI/2);
		shape.graphics.arc(rad/2,-hei/2,rad,Math.PI,-Math.PI/2);
		shape.graphics.closePath();
		shape.rotation=35;
		return shape;
	},

	tickShade:function(len,sCol,fCol){
		var hei=len;
		var wid=len/3;
		var rad=len/5;
		var shape=new createjs.Shape();
		...
		shape.graphics.arc(wid/2,-hei/2,rad,-Math.PI/2,0);
		shape.graphics.arc(wid/2,hei/2,rad,0,Math.PI/2);
		shape.graphics.arc(-wid/2-rad/2,hei/2,rad,Math.PI/2,Math.PI);
		shape.graphics.arc(-wid/2-rad/2,hei/2-wid/2+rad/3,rad,Math.PI,-Math.PI/2);
		shape.graphics.arc(-rad/1.3,hei/2-wid/2-rad/1.5,0,0,Math.PI/2);
		shape.graphics.arc(rad/4,-hei/2,rad,Math.PI,-Math.PI/2);
		shape.graphics.closePath();
		shape.rotation=35;
		return shape;
	},

	cross:function(wid,len,rad,sCol,fCol){
		var shape=new createjs.Shape();
		shape.graphics.beginFill(fCol).beginStroke(sCol);
		shape.graphics.arc(wid/2,-len-wid/2,rad,-Math.PI/2,0);
		shape.graphics.arc(wid/2+rad,-wid/2-rad,0,-Math.PI/2,0);
		shape.graphics.arc(wid/2+len,-wid/2,rad,-Math.PI/2,0);
		shape.graphics.arc(wid/2+len,wid/2,rad,0,Math.PI/2);
		shape.graphics.arc(wid/2+rad,wid/2+rad,0,0,Math.PI/2);
		shape.graphics.arc(wid/2,len+wid/2,rad,0,Math.PI/2);
		shape.graphics.arc(-wid/2,len+wid/2,rad,Math.PI/2,Math.PI);
		shape.graphics.arc(-wid/2-rad,wid/2+rad,0,Math.PI/2,Math.PI);
		shape.graphics.arc(-wid/2-len,wid/2,rad,Math.PI/2,Math.PI);
		shape.graphics.arc(-wid/2-len,-wid/2,rad,Math.PI,-Math.PI/2);
		shape.graphics.arc(-wid/2-rad,-wid/2-rad,0,Math.PI,-Math.PI/2);
		shape.graphics.arc(-wid/2,-wid/2-len,rad,Math.PI,-Math.PI/2);
		shape.graphics.closePath();
		shape.rotation=45;
		return shape;
	},

	rectangle:function($wid,$hei,$rnd,sCol,fCol){
		var shape=new createjs.Shape();
		shape.graphics.beginFill(fCol).beginStroke(sCol);
		shape.graphics.drawRoundRect(-$wid/2,-$hei/2,$wid,$hei,$rnd);
		return shape;
...
var OpdTimer=function($callback){
	var timeout=null;
	var started=null;
	var remaining=null;
...
(function(oG){
	...
	p.setup=function(){
		...
		var tickIm=opdShapes.tick(50,'#333','green');
		var crossIm=opdShapes.cross(10,20,4,'#333','red');
		...
		opdLib.dispItem(crossIm,this.cross,0,0);
		opdLib.dispItem(tickIm,this.tick,0,0);
	};
...
//other functions maybe add to lib


	p.arrangePhrasals=function(){
		//this shuffles an array within certain bounds
		//window is the limit to which an item can vary n cannot be shuffled
		//more than n+window.  eg with window 3 the value 0 must be in position 0,1or2
		//range is the amount of variance, more is more random, less is less repetitive

		//one problem with this is that it undoes the certainty that of no adjacent
		//repeats that the myShuffle ensured - so, it's possible that 2 same options
		//can appear at the same time - so, the correct check needs to be based on
		//the text and not the ind

		var nArr=[];
		var i=0;
		var tVar=0;
		var aLen=this.routineArr.length;

		//set first 2
		nArr[0]=Math.floor(Math.random()*3);
		do{
			nArr[1]=Math.floor(Math.random()*4)
		}while(nArr[1]==nArr[0])

		//set middle
		var prev=[-1,-1,-1,nArr[0],nArr[1]];
		var loopInd=0;
		for(i=2;i<aLen-3;i++){
			if(!doesArrayContain(loopInd,prev)){
				tVar=loopInd;
			}else{
				do{
					tVar=loopInd+Math.ceil(Math.random()*5);
				}while(doesArrayContain(tVar,prev));
			}
			for(var j=0;j<prev.length-1;j++)prev[j]=prev[j+1];
			prev[prev.length-1]=tVar;
			nArr[i]=tVar;
			loopInd++
		}

		//set last 3 by finding any missing values
		for(i=aLen-5;i<aLen;i++){
			if(!doesArrayContain(i,prev)){
				nArr.push(i);
			}
		}
		...
		var out=[];
		for(i=0;i<aLen;i++){
			out[i]=this.routineArr[nArr[i]];
		}
		return out;
	};

	function doesArrayContain(item,array){
		for(var i=0;i<array.length;i++){
			if(item==array[i])return true;
		}
		return false;
...
(function(oG){//checked
	...
	p.setup=function(){
		this.showTextDelay=new OpdTimer(this.playTextFun);
		this.showAudioDelay=new OpdTimer(this.playAudioFun);
		this.autoPlayDelay=new OpdTimer(this.autoPlayFun);
...
(function(oG){
	var RotateCheck=function(imSrc,contDiv){
		this.Container_constructor();
		this.rotateImSrc=imSrc;
		this.contDiv=contDiv;
		this.exitClickFun=this.exitClick.bind(this);
		this.dummyFunFun=this.dummyFun.bind(this);
		this.loadedFun=this.loaded.bind(this);
		this.exitCallback=function(){};
		this.setup();
	};

	var p=createjs.extend(RotateCheck,createjs.Container);
	...
	p.showPane=function(){
		this.fullShade=new createjs.Shape();
		this.fullShade.graphics.beginFill('#fff').drawRect(0,-25,800,600);
		this.fullShade.alpha=0.1;
		this.addChild(this.fullShade);

		var centerShade=new createjs.Shape();
		centerShade.graphics.beginFill('#ccc').setStrokeStyle(3).beginStroke('#666').drawRoundRect(100,100,600,350,24);
		this.addChild(centerShade);
		this.timeOut=null;

		this.exitBut=new createjs.Container();
		var backCirc=new createjs.Shape();
		backCirc.graphics.beginFill('#666');
		backCirc.graphics.drawCircle(0,0,50);
		var crossLine=new createjs.Shape();
		crossLine.graphics.setStrokeStyle(3).beginStroke('#333');
		crossLine.graphics.moveTo(-26,-26);
		crossLine.graphics.lineTo(26,26);
		crossLine.graphics.moveTo(26,-26);
		crossLine.graphics.lineTo(-26,26);

		this.exitBut.addChild(backCirc);
		this.exitBut.addChild(crossLine);

		opdLib.dispItem(this.exitBut,this,640,160);

		this.rotateText=new createjs.Text('Rotate Screen','bold 56px Arial','#333');
		opdLib.centerText(this.rotateText);
		opdLib.dispItem(this.rotateText,this,400,290);

		this.myIm=new Image();
		this.myIm.addEventListener('load',this.loadedFun);
		this.myIm.src=this.rotateImSrc;
	};

	p.loaded=function(){
		this.removeChild(this.rotateText);
		this.myIm.removeEventListener('load',this.loadedFun);
		this.myBitIm=new createjs.Bitmap(this.myIm);
		opdLib.dispItem(this.myBitIm,this,230,165);
	};

	p.check=function(){
		var mob=false;
		if(createjs.BrowserDetect.isIOS)mob=true;
		if(createjs.BrowserDetect.isAndroid)mob=true;
		if(!mob)return false;
		var winHei=window.innerHeight;
		var winWid=document.getElementById(this.contDiv).clientWidth;
		if(winHei<winWid)return false;
		return true;
	};

	p.setExitCallback=function(leFun){
		this.exitCallback=leFun;
	};
	...
	p.resizeCheck=function(){
		if(this.showing===true)if(!this.check())oG.view.rotateOff();
	};
	...
	p.init=function(){
		...
		this.exitBut.addEventListener('click',this.exitClickFun);
		this.exitBut.cursor='pointer';
		this.fullShade.addEventListener('click',this.dummyFunFun);
		this.fullShade.addEventListener('mouseover',this.dummyFunFun);
		this.fullShade.addEventListener('mouseout',this.dummyFunFun);
	};

	p.deit=function(){
		this.showing=false;
		this.exitBut.removeEventListener('click',this.exitClickFun);
		this.exitBut.cursor='default';
		this.fullShade.removeEventListener('click',this.dummyFunFun);
		this.fullShade.removeEventListener('mouseover',this.dummyFunFun);
		this.fullShade.removeEventListener('mouseout',this.dummyFunFun);
		this.myIm.removeEventListener('load',this.loadedFun);
	};

	oG.Modules.RotateCheck=createjs.promote(RotateCheck,'Container');
}(opdGame));
