...
opdGame.init=function(){
	console.log(opdGame.settings.version);

	//serverMets
	opdGame.serverMetrics.saveProgress(0);
	console.log('sent metrics 0');

	opdGame.canvasSize.setup();

	opdGame.stage=new createjs.Stage('myCanvas');
	createjs.Sound.initializeDefaultPlugins();
	opdGame.stage.enableMouseOver(10);
	opdGame.stage.snapToPixelEnabled=true;
	opdGame.stage.mouseMoveOutside=true;
	createjs.Touch.enable(opdGame.stage);
	createjs.Ticker.setFPS(30);
	createjs.Ticker.addEventListener("tick",opdGame.stage);

	if(createjs.BrowserDetect.isIOS){
		opdGame.settings.touchMode=true;
		opdGame.settings.userContentOn=false;
	}
	...
	if(createjs.Sound.capabilities.ogg===false)opdGame.settings.audOn=false;

	opdGame.view=new opdGame.View();

	document.getElementById('fileIn').onclick=function(){this.value=null;};

	opdGame.stage.addChild(opdGame.view);

	opdGame.settings.siteLock=false;
	//opdGame.settings.userContentOn=false

	opdGame.canvasSize.setCallback(opdGame.view.orientationChange);
	opdGame.canvasSize.setCallback2(opdGame.view.updateResize);
...
(function(oL){
	...
	function windowScroll(){
		if(!scrollWaiting){
			scrollWaiting=true;
			scrollTimeout=setTimeout(winScroll,500);
		}
	}

	function winScroll(){
//don't do this if can height is less than win height - only if it's the same so needs to adjust and prevent off screen ness
		var pageY=window.pageYOffset;
		var canY=document.getElementById("myCanvas").offsetTop;
		var canMinY=canY-40;
		var canMaxY=canY+40;
	...
	function setWinDimensions(){
		...
		winWid=document.getElementById('containerDiv').clientWidth;
	}
	...
	function stretchCanvas(){
		...
		if(winWid/winHei>aspectRatio){
			//hei=winHei-24;//don't need this as using sticky to auto adjust
			hei=winHei;
			wid=hei*aspectRatio;
		}else{
			wid=winWid;
...
//only do this if dimens are near enough
		if(!fullScreen){
	...
	function windowResize(){
		if(!waiting){
			waiting=true;
			waitingTimeout=setTimeout(winResize,700);
		}
	}
	...
	function setCallbackLoc($callBack){orientationChangeCallback=$callBack;}

	function setCallback2Loc($callBack){resizeCallback=$callBack;}
...
(function(oG){
	...
	function getScoresLoc(getType,playerName,playerLocation){
		...
		myReq.open('POST',oG.settings.scoresUrl,true);
		...
		myVars+='&table='+oG.settings.scoresTable;
		myVars+='&inBool='+getType;
		myVars+='&gameType='+oG.settings.gameType;
...
(function(oG){//checked
	...
	p.setupDisplay=function(){
		if(oG.canvasSize.getOrientation()===0){
...
(function(oG){//checked
	...
	function initLoc(){
		//$('#fileIn').on('change',fileInputEvent);
		document.getElementById('fileIn').addEventListener('change',fileInputEvent);
		...
		if(oG.settings.getPresetHash===true)hashCheck();
	}
	...
	//function openFileDialogLoc(){$('#fileIn').trigger('click');}
	function openFileDialogLoc(){document.getElementById('fileIn').click();}
...
(function(oG){//checked
	...
	p.setup=function(){
		...
		this.imFolder=oG.settings.contentFolder+'images/use/';
		this.auFolder=oG.settings.contentFolder+oG.settings.audioFolder;
		this.deFolder=oG.settings.contentFolder+'descText/';
...
(function(oG){//checked
	...
	p.setup=function(){
		...
		if(!oG.settings.userContentOn)this.myShapesHolder.removeChild(this.myShapes[20]);
	...
	p.posShapes=function(){
		...
		if(oG.canvasSize.getOrientation()===0){
			var ini=SHAPE_INI[1];
			if(oG.settings.userContentOn===false)ini=SHAPE_INI[2];
			for(i=0;i<5;i++){
	...
	p.contentItemClicked=function(event){
		...
		if(cVar==20){
			if(oG.settings.userContentOn)oG.contentController.addClicked();
		}else{
			this.displayLoading();
...
(function(oG){
	...
	p.setup=function(){
		...
		if(!oG.settings.userContentOn)this.removeChild(this.contentBits[5]);
...
(function(oG){//checked
	...
	p.setup=function(){
		...
		this.uDiv=document.getElementById('inputDivContent');
		this.uObj=new createjs.DOMElement('inputDivContent');
	...
	p.updateNumbers=function(){
		...
		if(num>=oG.settings.contentMin){
			this.menu.infoText_2.text='max : '+oG.settings.contentMax;
			this.menu.enableDoneButs();
		}else{
			this.menu.infoText_2.text='min : '+oG.settings.contentMin;
	...
	p.keyPress=function(e){
		if(oG.settings.lettersOnly)this.removeNonLetters();
		this.limInput();
		if(e.which==13){
			this.outClick(null);
		}
	...
	p.positionUserInput=function(){
		var rat=oG.canvasSize.getCanvasRatio();
...
///////////////adding images stuff
	p.loadFileImage=function(iFile){
		...
		if(iInd<oG.settings.contentMax){
...
(function(oG){//checked
	...
	function initLoc(){
		...
		if(window.location.host===oG.settings.siteUrl){
			oG.view.init();
		}else{
			if(oG.settings.siteLock===false){
				//oG.settings.offSite=true;
				oG.view.init();
			}
		}
	}
	...
	function updateResizeFun(){
		if(oG.model.preloadComplete){
			oG.view.endView.myHSControls.updateInputs();
...
(function(oG){//checked
	...
	p.setup=function(){
		...
		this.inputPane=new oG.Modules.NameInputPane();

		this.addChild(this.myScoresTable);
		this.addChild(this.inputPane);

		this.addChild(this.exitBut);
		this.addChild(this.againBut);

		this.inputPane.setCallback(this.sendNewHighScore);
	...
	p.setupDisplay=function(){
		if(oG.canvasSize.getOrientation()===0){
			opdLib.posItem(this.exitBut,400,474);
			opdLib.posItem(this.againBut,400,510);
		}else{
			opdLib.posItem(this.exitBut,275,724);
			opdLib.posItem(this.againBut,275,760);
		}
	};
	...
	p.orientationChange=function(){
		...
		this.inputPane.setupDisplay();
	};

	p.sendNewHighScore=function(nom,local){
		oG.scoresModel.getScores(1,nom,local);	
		oG.canvasSize.unlock();
		oG.view.endView.myScoresTable.bringDown();
	};

	p.gotScoresFun=function(jsonArr){this.myScoresTable.showScores(jsonArr);};

	p.init=function(){
		opdLib.fadeIn(this,500,200);
		opdLib.fadeIn(this.exitBut,500,700);
		opdLib.fadeIn(this.againBut,500,700);
		opdLib.fadeIn(this.inputPane,500,700);
		...
		if(oG.model.gameScore>0){
			oG.canvasSize.lock();
			this.inputPane.visible=true;
			this.inputPane.init();
		}else{
			this.inputPane.visible=false;
		}
	};
...
(function(oG){//checked
	function GameView(){
		...
		this.reAddTapFun=this.reAddTap.bind(this);
	...
	p.settingsClick=function(event){
		...
		this.tapDelay=setTimeout(this.reAddTapFun,350);
...
(function(oG){//checked
	...
	var titleLabels=['Name','Score','Date','Location'];
	var xInd=[75,200,304,416];
	...
	p.setup=function(){
		...
		this.backPane.alpha=0.2;

		this.titleText=new createjs.Text('','bold 26px Reem Kufi','#666');
		...
		this.tFields=[];
		for(var i=0;i<4;i++){
			var title=new createjs.Text(titleLabels[i],'bold 30px Reem Kufi', '#666');
			this.titlesContainer.addChild(title);
			title.x=xInd[i];
			opdLib.centerText(title);

			this.tFields[i]=new createjs.Text('','bold 22px Reem Kufi', '#666');
			this.fieldsContainer.addChild(this.tFields[i]);
			this.tFields[i].maxWidth=150;
			this.tFields[i].lineHeight=LINE_HEIGHT;
			this.tFields[i].x=xInd[i];
		...
		this.myUpBut=opdLib.drawArrow(20,'#ff8');
		this.myDownBut=opdLib.drawArrow(20,'#ff8');
		this.myUpBut.rotation=-90;
		this.myDownBut.rotation=90;
		this.myUpBut.cursor='pointer';
		this.myDownBut.cursor='pointer';

		this.addChild(this.titlesContainer);
		this.addChild(this.fieldsContainer);
		this.addChild(this.titleText);

		this.addChild(this.myUpBut);
		this.addChild(this.myDownBut);
	...
	p.setupDisplay=function(){
		if(oG.canvasSize.getOrientation()===0){
			...
			this.backPane.graphics.clear().beginFill('#fff').drawRoundRect(150,20,524,426,24);
			opdLib.posItem(this.titleText,415,64);
			opdLib.posItem(this.titlesContainer,168,109);
			this.baseY=151;
			this.fieldsX=168;
			opdLib.posItem(this.myUpBut,720,200);
			opdLib.posItem(this.myDownBut,720,310);
		}else{
			this.fieldsMask.graphics.clear().beginFill('#000').drawRect(35,260,600,296);
			this.backPane.graphics.clear().beginFill('#fff').drawRoundRect(13,150,524,426,24);
			opdLib.posItem(this.titleText,278,194);
			opdLib.posItem(this.titlesContainer,31,239);
			this.baseY=281;
			this.fieldsX=31;
			opdLib.posItem(this.myUpBut,275,60);
			opdLib.posItem(this.myDownBut,275,130);
		}
		opdLib.posItem(this.fieldsContainer,this.fieldsX,this.baseY);
		this.setMinY();
	};

	p.bringDown=function(){
		if(oG.canvasSize.getOrientation()===1){
			this.fieldsMask.graphics.clear().beginFill('#000').drawRect(35,310,600,296);
			this.backPane.graphics.clear().beginFill('#fff').drawRoundRect(13,200,524,426,24);
			opdLib.posItem(this.titleText,278,244);
			opdLib.posItem(this.titlesContainer,31,279);
			this.baseY=331;
			opdLib.posItem(this.fieldsContainer,this.fieldsX,this.baseY);
			this.setMinY();
		}
	...
	p.showScores=function(jsonArr){
		var i=0;
		for(i=0;i<4;i++)this.tFields[i].text='';
		var myDate=new Date();
		this.rowsCount=jsonArr.scores.length;
		for(i=0;i<this.rowsCount;i++){
			myDate.setTime(jsonArr.scores[i].Dote);
			var myMon=monthText[myDate.getMonth()];
			var myDay=myDate.getDate();
			this.tFields[0].text+=jsonArr.scores[i].Nom+'\r\n';
			this.tFields[1].text+=jsonArr.scores[i].Score+'\r\n';
			this.tFields[2].text+=myDay+'-'+myMon+'\r\n';
			this.tFields[3].text+=jsonArr.scores[i].Local+'\r\n';
		}
		this.setMinY();
	};
...
(function(oG){//checked
	...
	function ImagePane(){
		...
		this.shortTapDelayFun=this.shortTapDelay.bind(this);
		this.setup();
	}
	...
	p.routine=function($curArray){
		...
		if(!oG.settings.touchMode){
			this.addItemLists();
		}else{
			this.tapTimeout=setTimeout(this.shortTapDelayFun,350);
		}
	};

	p.shortTapDelay=function(){
		this.addItemLists();
	};
...
(function(oG){//checked
	...
	p.ticker=function(){
		...
		if(this.fadeOutVar&&oG.settings.audOn){
			this.fadeOutVar--;
		...
		if(this.fadeInVar&&oG.settings.audOn){
			this.fadeInVar--;
...
(function(oG){//checked
	...
	var p=createjs.extend(NameInputPane,createjs.Container);
	...
	p.setup=function(){
		...
		this.scoreDisp=new createjs.Text('0','bold 16px Reem Kufi','#333');

		this.entryPaneBack=new createjs.Shape();
		this.entryPane=new createjs.Shape();
		this.submitText=new createjs.Text('Submit','bold 18px Reem Kufi','#444');
		this.nameLabel=new createjs.Text('Name','bold 16px Reem Kufi','#333');
		this.localLabel=new createjs.Text('Location','bold 16px Reem Kufi','#333');
		this.scoreLabel=new createjs.Text('Score','bold 16px Reem Kufi','#333');

		this.entryPaneContainer.addChild(this.entryPaneBack);
		this.entryPaneContainer.addChild(this.entryPane);
		...
		this.entryPaneContainer.addChild(this.submitBut);
		this.entryPaneContainer.addChild(this.nameLabel);
		this.entryPaneContainer.addChild(this.localLabel);
		this.entryPaneContainer.addChild(this.scoreLabel);
		this.entryPaneContainer.addChild(this.scoreDisp);

		this.addChild(this.entryPaneContainer);
		...
		this.nDiv=document.getElementById('inputDivName');
		this.lDiv=document.getElementById('inputDivLocat');
		this.nDiv.style.textAlign='center';
		this.lDiv.style.textAlign='center';
		this.nObj=new createjs.DOMElement('inputDivName');
		this.lObj=new createjs.DOMElement('inputDivLocat');
	...
	p.setupDisplay=function(){
		...
		var paneWidth=120;
		if(oG.canvasSize.getOrientation()===0){
			opdLib.posItem(this.entryPaneContainer,90,250);

			opdLib.posItem(this.submitBut,0,140);
			opdLib.posItem(this.nameLabel,0,-60);
			opdLib.posItem(this.localLabel,0,0);
			opdLib.posItem(this.scoreLabel,0,55);
			opdLib.posItem(this.scoreDisp,0,80);
			...
			opdLib.posItem(this.entryPaneContainer,275,640);

			opdLib.posItem(this.submitBut,0,-50);
			opdLib.posItem(this.nameLabel,-110,-8);
			opdLib.posItem(this.localLabel,0,-8);
			opdLib.posItem(this.scoreLabel,110,-8);
			opdLib.posItem(this.scoreDisp,110,18);
		...
		var wid=paneWidth-12;
		var hei=paneHeight-12;

		this.entryPaneBack.graphics.beginFill('#ff8').drawRoundRect(-paneWidth/2,-paneHeight/2,paneWidth,paneHeight,16);
	...
	p.setCallback=function(newFun){
		this.submitCallback=newFun;
	};
	...
	p.init=function(){
		this.active=true;
		this.scoreDisp.text=oG.model.gameScore;
		document.addEventListener('keyup',this.keyPressFun);
		this.entryPaneContainer.visible=true;
		this.submitBut.addEventListener('click',this.submitClickFun);
		this.showFieldsTimeout=setTimeout(this.showFieldsFun,300);
		this.updateInputs();
	};
	...
	p.deit=function(){
		this.active=false;
		this.endSubmit();
		clearTimeout(this.showFieldsTimeout);
		document.removeEventListener('keyup',this.keyPressFun);
	};

	oG.Modules.NameInputPane=createjs.promote(NameInputPane,'Container');
}(opdGame));
...
(function(oG){//checked
	...
	var myManifest=[];
	myManifest.push({src:'mySprite.png',id:'allSprite'});
	if(oG.settings.audOn===true){
		myManifest.push({src:'audioSprite.ogg',data:{audioSprite:aSpriteData}});
	}
	...
	function load(){
		...
		myLoadQueue.loadManifest(myManifest,true,oG.settings.resFolder);
	}
...
(function(oG){//checked
	...
	p.setup=function(){
		...
		preLoaderImage.src=oG.settings.preLoaderImageSrc;
...
(function(oG){//checked
	function ReviewView(){
		...
		this.settingsClickFun=this.settingsClick.bind(this);
		this.tapCheckFun=this.tapCheck.bind(this);
		this.reAddFun=this.reAdd.bind(this);
		this.reAddSettingsFun=this.reAddSettings.bind(this);
		this.setup();
	}
	...
	p.tapCheck=function(){
		if(oG.settings.touchMode){
			...
			this.tapTimeout=setTimeout(this.reAddFun,350);
		}
	};
	...
	p.settingsTapCheck=function(){
		this.settingsBut.removeEventListener('click',this.settingsClickFun);
		this.settingsTimeout=setTimeout(this.reAddSettingsFun,350);
	};

	p.reAddSettings=function(){
		this.settingsBut.addEventListener('click',this.settingsClickFun);
		if(this.settingsPane.visible==true){
			this.settingsPane.exitBut.addEventListener('click',this.settingsClickFun);
			this.settingsPane.fullBack.addEventListener('click',this.settingsClickFun);
		}
	};
...
(function(oA){
	var url='https://www.spanishinflow.com:8080/';
	var myId=0;
...
(function(oG){//checked
	...
	p.mouseOverer0=function(){
		if(oG.canvasSize.getOrientation()===0)this.myMover([yArr[0],yArr[1]+4,yArr[2]+4,yArr[3]+4],[1.2,1,1,1]);
	};

	p.mouseOverer1=function(){
		if(oG.canvasSize.getOrientation()===0)this.myMover([yArr[0]-4,yArr[1],yArr[2]+4,yArr[3]+4],[1,1.2,1,1]);
	};

	p.mouseOverer2=function(){
		if(oG.canvasSize.getOrientation()===0)this.myMover([yArr[0]-4,yArr[1]-4,yArr[2],yArr[3]+4],[1,1,1.2,1]);
	};

	p.mouseOverer3=function(){
		if(oG.canvasSize.getOrientation()===0)this.myMover([yArr[0]-4,yArr[1]-4,yArr[2]-4,yArr[3]],[1,1,1,1.2]);
	};

	p.mouseOut=function(){
		if(oG.canvasSize.getOrientation()===0)this.myMover(yArr,[1,1,1,1]);
	};
	...
	p.addLists=function(){
		for(var i=0;i<4;i++){
			...
			this.butsArr[i].addEventListener('mouseout',this.mouseOutFun);
		}
	...
	p.removeLists=function(){
		for(var i=0;i<4;i++){
			...
			this.butsArr[i].removeEventListener('mouseout',this.mouseOutFun);
		}
	...
	p.init=function(){
		if(oG.settings.offSite)this.reviewBut.alterButton('textGames','games');
...
(function(oG){
	...
	p.clicker=function(event){
		...
		if(this.clickTarget=='games'){
			var redirectWindow=window.open(oG.settings.siteUrlFull,'_blank');
...
(function(oG){//checked
	...
	p.updateResizeFun=function(){
		if(oG.model.preloadComplete){
			oG.view.endView.inputPane.updateInputs();
			oG.contentController.resizeInputBit();
		}
	};
